...
try:
    ...
    while True:
        Forwards()
        time.sleep(0.1)
		leftOrRight = random.randrange(2)
		
		while(IsNearObstacle(HowNear) and leftOrRight == 0):
			StopMotors()
			Left()
			time.sleep(0.5)
			StopMotors()
			
		while(IsNearObstacle(HowNear) and leftOrRight == 1):
			StopMotors()
			Right()
			time.sleep(0.5)
			StopMotors()
